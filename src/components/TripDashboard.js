  ...
  useEffect(() => {
    const fetchTripDetails = async (tripId) => {
      try {
        ...
        if (data && data.trip) {
          console.log("Setting trip from data.trip:", data.trip);
          setTrip(data.trip);
          ...
          if ("tripName" in data || "_id" in data || "startDate" in data) {
            console.log("Setting trip from root data object:", data);
            setTrip(data);
          } else if (data.trips && data.trips.length > 0) {
            // In case the API returns an array of trips
            console.log(
              "Setting trip from first item in data.trips array:",
              data.trips[0]
            );
    ...
    if (tripId) {
      console.log("Initiating fetch for trip ID:", tripId);
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      >
        ...
        >
          ...
          >
            <Stack direction="column" spacing={1} sx={{ mb: { xs: 2, sm: 0 } }}>
